...
from tasks.serializers import (
    TaskSerializer, TaskCreateSerializer, TaskUpdateSerializer, TaskDeleteSerializer
)
    ...
    try:
        ...
        if tasks.exists():
            ...
            if serializer.is_valid():
                ...
                return Response({'success': False, 'data': None, 'msg': 'Falha na serialização'}, status=status.HTTP_400_BAD_REQUEST)
        else:
    ...
    try:
        ...
        if serializer.is_valid():
            ...
            if instance:
                ...
                return Response({'success': False, 'data': None, 'msg': 'Falha ao criar tarefa'}, status=status.HTTP_400_BAD_REQUEST)
        else:
...
def tasks_delete_task(request, task_id):
    try:
        task = Task.objects.get(id=task_id, user=request.user)
        serializer = TaskDeleteSerializer(task)
        if serializer.is_valid():
            task.delete()
            return Response({'success': True, 'data': serializer.data, 'msg': 'Tarefa deletada com sucesso'}, status=status.HTTP_200_OK)
        else:
            return Response({'success': False, 'data': None, 'msg': 'Erro de serialização'}, status=status.HTTP_400_BAD_REQUEST)
    except Task.DoesNotExist:
        return Response({'success': False, 'data': None, 'msg': 'Tarefa não encontrada'}, status=status.HTTP_404_NOT_FOUND)
